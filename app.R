...
ui <- fluidPage(
    ...
    fluidRow(
        ...
        column(2, downloadButton(outputId = 'download', label = 'download combined transcriptome for selected tissues'))
    ),
    ...
    output$tx_diag <- renderUI({
        ...
        if(length(s_keep_tx) == 0){
            ...
            output$download <- downloadHandler(
                filename = function() {paste0(paste(s_tissues, collapse = '-'), '.Rdata') },
